...
Regression, Romano et al. 2019, arXiv:1905.03222). The three quantile models
are trained independently and their raw 80% interval under-covers. Wrapping
them with a held-out calibration set restores a finite-sample,
distribution-free coverage guarantee while keeping the interval adaptive to
each row.

The calibration set must come from the period after training and before the
evaluation window, otherwise the guarantee does not hold.
"""
