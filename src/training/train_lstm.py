"""
Train LSTM model for delay prediction with weather features.
Saves model checkpoint, scaler, and feature list to trained_models/.
"""
...
def create_sequences_by_date(df, feature_cols, target_col, scaler,
    ...
    """Create sequences for LSTM. Each sequence uses only past data (no leakage)."""
    sequences_X = []
    sequences_y = []
    ...
    for route in df["route"].unique():
        ...
        for idx in range(len(route_df)):
            ...
            # X is past 28 days, y is today (no overlap)
...
def main():
    print("Training LSTM Delay Model with Weather Features...")
    ...
    # 22-feature set, no explicit lags since the LSTM sees them in the sequence
    ...
    model_path = MODELS_DIR / "best_lstm_arr_delay.pt"
    torch.save({
    ...
    scaler_path = MODELS_DIR / "feature_scaler_arr_delay.pkl"
