...
def objective(trial, X_train, y_train, X_val, y_val):
    """Optuna objective: train LightGBM with sampled params, return val MAE."""
    params = {
        "n_estimators": trial.suggest_int("n_estimators", 200, 1000, step=50),
        "num_leaves": trial.suggest_int("num_leaves", 15, 127),
        "learning_rate": trial.suggest_float("learning_rate", 0.01, 0.3, log=True),
        "subsample": trial.suggest_float("subsample", 0.6, 1.0),
        # 0 disables bagging entirely, which makes subsample inert, and letting
...
def main():
    """Runs Optuna study for LightGBM and saves best params."""
    np.random.seed(42)
    ...
    output_path = CONFIGS_DIR / "best_params_lightgbm.json"
    ...
    with tracking.start_run(run_name="tune_lightgbm", tags=provenance):
